...
- `ObserverPictureInPicture` performs a bounded 10 FPS Observer render before the normal player-world pass, refreshes camera uniforms on both sides of the temporary switch, asynchronously downsizes the captured frame, and displays it as a small live HUD texture. It is off by default and releases that texture on the render thread.
...
`ObserverRecordingManager` owns a four-state recording session: idle, starting, recording, and finalizing. Recording can start only in Observer POV. It stops safely on POV/world loss, framebuffer resize, shutdown, encoder failure, low free space, or the configured folder cap. A session token rejects late asynchronous callbacks from older recordings.
